...
mse2psnr = lambda x : -10. * torch.log(x).to(device) / torch.log(torch.Tensor([10.]).to(device)).to(device)
...
# Model
class EvNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=2, skips=[4], use_viewdirs=False, device='cpu'):
        """ 
        """
        super(EvNeRF, self).__init__()
        self.D = D
        self.W = W
        ...
        self.pts_linears = nn.ModuleList(
            [nn.Linear(input_ch, W).to(device)] + [nn.Linear(W, W).to(device) if i not in self.skips else nn.Linear(W + input_ch, W).to(device) for i in range(D-1)])
        
        ### Implementation according to the official code release (https://github.com/bmild/nerf/blob/master/run_nerf_helpers.py#L104-L105)
        self.views_linears = nn.ModuleList([nn.Linear(input_ch_views + W, W//2).to(device)])
        ...
        if use_viewdirs:
            self.feature_linear = nn.Linear(W, W).to(device)
            self.alpha_linear = nn.Linear(W, 1).to(device)
            self.gray_linear = nn.Linear(W//2, 1).to(device)
        else:
...
def get_rays(H, W, K, c2w):
    ...
    rays_o = c2w[:3,-1].expand(rays_d.shape).to(device)
    return rays_o, rays_d.to(device)


def get_rays_np(H, W, K, c2w, device):
    ...
    return torch.from_numpy(rays_o).to(device), torch.from_numpy(rays_d).to(device)
...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    cdf = torch.cat([torch.zeros_like(cdf[...,:1]), cdf], -1).to(device)  # (batch, len(bins))
    ...
    inds_g = torch.stack([below, above], -1).to(device)  # (batch, N_samples, 2)
...
def img_from_events(events, width, height):
    """
